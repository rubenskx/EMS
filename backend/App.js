  ...
  for (let i = 0; i < data.length; i++) {
    try {
      ...
      if (
        ...
        const basic = Math.round(data[i].current_salary / divider);
        const query = `INSERT INTO employee_data (id,name,mobile_no,gender,date_of_joining,qualification,previous_experience,year_of_course_completion,retired,current_salary,wef,deduction,remarks,head_engineer,director,email,department_id,project_id,current_designation_id ,previous_designation_id,Basic_Salary,hra,da) VALUES ("${data[i].id}", "${data[i].name}","${data[i].mobile_no}", "${data[i].gender}", "${data[i].date_of_joining}","${data[i].qualification}","${data[i].previous_experience}",${data[i].year_of_course_completion},"${data[i].retired}",${data[i].current_salary},"${data[i].wef}",${data[i].deduction},"${data[i].remarks}","${data[i].head_engineer}","${data[i].director}","${data[i].email}",${deptRes[0].department_id},${projRes[0].project_id},${currDesigRes[0].designation_id}, ${prevDesigRes[0].designation_id},${basic},${HRA},${DA}); INSERT INTO salary(salary,wef_date,status,employee_id) VALUES(${data[i].current_salary},"${data[i].wef}","current", "${data[i].id}");`;
        console.log(query);
        const result = await queryDatabase(query);
...
app.post("/add/hra", async (req, res) => {
  ...
  empData.forEach(async (record) => {
    ...
    await queryDatabase(
      `UPDATE employee_data SET current_salary=${newSalary},hra=${newHRA} WHERE id="${record.id}"`
    );
    await queryDatabase(
...
app.post("/add/da", async (req, res) => {
  ...
  empData.forEach(async (record) => {
    ...
    const newSalary = basic * newdivider;
    await queryDatabase(
      `UPDATE employee_data SET current_salary=${newSalary},da=${newDA} WHERE id="${record.id}"`
    );
    await queryDatabase(
      `UPDATE salary SET salary=${newSalary} WHERE employee_id="${record.id}" AND status="current"`
    );
